...
The DWLS function $f_{ls}(\bx)$ is a quasi-interpolant because it does not exactly agree with the function values at each data point.  The residual at the data points $r_i = f_i - f_{ls}(\bx_i)$ will be represented by a Gaussian process kriging model $f_{k}(\bx)$.
\notes{put in a description of kriging here}

The surrogate model is then defined as $\SURR(n,\mX,\by)(\bx)=f_{ls}(x) + f_{k}(x)$.  
...
Let's mention UQLab \cite{UQLab2014}, emukit, \cite{emukit2019}, SUMO\cite{SUMO2010} ooDACE \cite{ooDACE2014}, CODES \cite{CODES2015}, and pySOT \cite{pySOT2015}.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The DWLS minimizes the residual $\sum w_i(f_i - f_{ls}(\bx_i))^2$, where the weights, $w_i(\bx)$ depend on the location where the function will be evaluated. In the Bayesian bootstrap method \cite{Rubin1981, Efron1982, Efron2016} we generate an ensemble of $B$ DWLS quasi-interpolants, $f_{ls}^b$, $b=1,B$. Each quasi-interpolant is determined by reweighted the DWLS weights by a factor 

these weight are reweighted by uniformly distributed samples $\bv$ the $n$ dimensional simplex.  That is, on each bootstrap integration we  
