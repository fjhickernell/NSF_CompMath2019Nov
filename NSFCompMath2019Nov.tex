...
To facilitate the analysis, we define $\QOI \colon \calf \to \calg$, as the map from the space of possible simulation input POIs, $\calf$,  to the space of possible output QOIs,  $\calg$.  Possible QOIs include surrogate models for global uncertainty analysis, optimal values of nonconvex response surfaces, and the integrals of multivariate high-dimensional functions.   For example,  
\begin{equation}
    \QOI(f) = f, \quad \QOI(f) = \min_{\bx \in \Omega} f(\bx), \quad \QOI(f) = \argmin_{\bx \in \Omega} f(\bx), \quad \QOI(f) = \int_{\Omega} f(\bx) \, \dif \bx.
\end{equation}
In these examples $\calg$ corresponds to a Banach space of functions, $\reals$, or $\Omega$.
...
The goal of the \emph{exploit} stage is to use all past samples to detect regions where $f$ is poorly represented with respect to its influence on $\QOI(f)$.  This information is then used to optimize the placement of future samples that reduce the uncertainty in the QOIs. 
...
\begin{itemize}
\item fitting kriging or reproducing kernel Hilbert space models where the parameter $\btheta$ determining the form of the  covariance/reporducing kernel is inferred from the data (Sect.\ \ref{sec:kerinferdata}), or 
...
The $\APP_{\btheta}(n_0, \mX, \by)$ are then used to  
\begin{itemize} 
\item identify the `best' approximation, $\APP(n, \mX, \by)$,

\item create a measure for the uncertainty of our approximation, $\ERR(n,\mX, \by)$ for a well defined set of functions $\calc$, as in \eqref{eq:errbd},  and 

\item estimate the expected value of each candidate sample, $\VAL(\bx,n,\mX, \by)$, to guide us in choosing the next data site, $\bx_{n+1}$, via \eqref{eq:nextsample}.
\end{itemize}
