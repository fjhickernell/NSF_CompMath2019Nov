...
We propose to find better $\APP$ and $\VAL$ than what is state-of-the art.  Current $\VAL$ do not assess the uncertainty of $\APP$ as well as they should, often because they do not take full account of $\by_{1:n}$
...
\end{equation}
where $\calc$ is a precisely defined set of maps between POIs and outputs.  This will allow the user to decide whether to additional data is needed to improve the accuracy of $\APP(n,\mX_{1:n},\by_{1:n})$.

We will use a two-step process to improve, create, and analyze adaptive sampling algorithms to sequentially guide the sample design based on both the locations and function values at the exiting samples.  

\subsection{Our plan}

Our plan follows the popular two-step \emph{explore-exploit} process.  In the \emph{explore} step, little or nothing is known of the QOIs or of $f$.  We sample $f$ on a relatively sparse, but space-filling, set of $n_0$ data sites over the region of feasible POIs.  This initial design must be large enough to detect important features of $f$ that influence $\QOI(f)$, but small enough to be economical. The initial design does not depend on function data.
...
For $\QOI(f) = f$, $\VAL(\bx,n,\mX_{1:n}, \by_{1:n})$ will correspond to a pointwise determinstic error bound or probabilistic error standard error.  For $\QOI(f) = \min_{\bx \in \Omega} f(\bx)$, $\VAL(\bx,n,\mX_{1:n}, \by_{1:n})$ may be determined from sensitivity analysis or correspond to expected improvement.

\subsection{Our impact.}
 
 The explore-exploit two-step process can sequentially guide the sampling process from the early stages, where little is know of the QOIs, though a stage where all past samples are exploited and each new sample is chosen to maximize the information gained.  
