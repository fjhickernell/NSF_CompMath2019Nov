...
\end{equation}
This surrogate model is known to have the following error bound:
\begin{align}
\label{RKHSErrBd}
...
Fig.\ \ref{fig:sampleFun} shows $\SURR(10,\mX,\by)$ for the univariate function $f \colon x \mapsto \exp(-10x) \sin(8x)$ using the $n=10$ point design, $\mX = (0, 0.1, \ldots, 0.6, 0.8, 0.9, 1)^T$.  
Although it is not yet sufficiently accurate for our purposes, $\SURR(10,\mX,\by)$ captures the peak on the left because the data sites are dense enough there.  If the design consists only of the $n=4$ sparse data sites, $\mX = (0, 0.4,  0.6, 1)^T$, then $\SURR(4,\mX,\by)$ misses the peak, as shown in Fig.\ \ref{fig:sampleFun}. Hence the need for the sufficient samples in the explore stage mentioned in Section \ref{sec:OurPlan}. 
...
We consider two QOIs, function approximation (the identity map), $\ID(f) = f$, and function minimization, $\MIN(f) = \min_{0 \le x \le 1} f(x)$.  For these two QOIs, our approximations are the surrogate model and the minimum function data observed:
\begin{equation*}
