...
\subsection{A Univariate Example of the RKHS/Kriging Approach}
...
\begin{itemize}
    \item The kernel, $K$, is not influenced by the output data, $\by$.
...
\end{itemize}
Remedies for this deficiency are are described in Sect.\ \ref{sec:kerinferdata}.
...
The approach outlined of minimizing prediction error outlined in Sect.\  \ref{sec:MinPredErr} that leads to the  cone of reasonable functions in  \eqref{RKHScone} and the error bound for the surrogate model in \eqref{eq:DataErrBd} has its faults. The error bound is far too large in some places and barely large enough in others.  Part of the reason is that the kernel $K$ chosen in \eqref{MatKer} and/or the value $\theta =1$ may not be the best suited to this particular $f$.  The function, $f$, that we intend to approximate belongs to an infinite number of function spaces or cones within those function spaces.  Our adaptive sampling scheme should help identify a function space for which $f$ is typical, not an unusual.
