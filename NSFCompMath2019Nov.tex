...
The univariate function $f: x \mapsto \exp(-10x) \sin(8x)$, plotted in Fig.\ \ref{fig:sampleFun}, illustrates some of the challenges of adaptive sampling.  Using the $n=10$ point design, $\mX = (0, 0.1, \ldots, 0.6, 0.8, 0.9, 1)^T$, an approximation, $\APP(f,n)$, is constructed as the minimum norm interpolant in the Hilbert space, $\calf$, defined by its Mat\'ern reproducing kernel,
...
\begin{equation} \label{minprederr}
    x_{n+1} = x_{\textup{bad}} = \argmax_x K(x,x) - \bk^T(x) \mK^{-1} \bk(x) = \phi_{n+1}(\mX),
\end{equation}
...
One approach to function approximation is to assume that the $f$ of interest belongs to a reproducing kernel Hilbert space $\calf$.  A commonly used kernel is the following member of the Mat\'ern kernel family,
\begin{equation} \label{MatKer}
...
\end{equation}
This surrogate model is known to have the following pointwise error bound:
\begin{align}
\label{RKHSErrBd}
    \abs{f(\bx) - \SURR(n,\mX,\by)(x)} & \le \sqrt{K(\bx,\bx) - \bk^T(\bx) \mK^{-1} \bk(\bx)} \, \norm[\calf]{f - \SURR(n,\mX,\by)} \\
    \nonumber
...
As an illustration, Fig.\ \ref{fig:sampleFun} displays the the univariate function $f \colon x \mapsto \exp(-10x) \sin(8x)$ and the corrsponding surrogate model $\SURR(10,\mX,\by)$ using the $n=10$ point design, $\mX = (0, 0.1, \ldots, 0.6, 0.8, 0.9, 1)^T$, and $\theta =1$.  
Although not highly accurate, $\SURR(10,\mX,\by)$ captures the peak on the left because the data sites are dense enough there.  If the design consists only of the $n=4$ sparse data sites, $\mX = (0, 0.4,  0.6, 1)^T$, then $\SURR(4,\mX,\by)$ misses the peak, as shown in Fig.\ \ref{fig:sampleFun}. This demonstrates the need for the sufficient samples in the explore stage mentioned in Section \ref{sec:OurPlan}. 
...
One common adaptive sampling rule for function approximation is to define the acquisition function, $\IDVAL$, in terms of the prediction error bound in \eqref{RKHSErrBd}, and then use the process suggested in \eqref{eq:nextsample}:
\begin{equation} \label{minprederr}
    \IDVAL(\bx,n,\mX,\by) =  K(\bx,\bx) - \bk^T(\bx) \mK^{-1} \bk(\bx), \qquad 
    \bx_{\ID, n+1} = \argmax_{\bx \in \Omega} \IDVAL(\bx,n,\mX,\by).
\end{equation}
Note that $\norm[\calf]{f - \SURR(n,\mX,\by)}$ is independent of $\bx$ and can be ignored here. The result of this rule for the example introduced above is plotted on the right in Fig.\ \ref{fig:sampleFun}.  However, $f(x_{\ID, 11}) \approx \SURR(11,\mX,\by)(x_{\ID, 11})$ so sampling at $x_{\ID, 11}$ does not help much.  The adaptive sampling rule in \eqref{minprederr} does not depend at all on the function data, $\by$, which is why it performs poorly.  Alternatives to this approach are described in Sections ???
...
\end{align}
where $A_n$ is positive, and fixed in advance.  This $\calc$ is a cone because if $f \in \calc$, then $c f \in \calc$ for any real $c$.  Noting that $\bignorm[\calf]{\SURR(n,\mX,\by)} = \sqrt{\by \mK^{-1} \by}$ error bound \eqref{RKHSErrBd} then implies an error bound that depends completely on function data: 
\begin{subequations} \label{DataErrBd}
\begin{gather}
    \label{DataErrBda} 
    \SURRERR(\bx,n,\mX,\by) : = A_n \sqrt{[K(\bx,x) - \bk^T(\bx) \mK^{-1} \bk(\bx)] \, [\by^T \mK^{-1} \by] }, \\
    \abs{f(\bx) - \SURR(n,\mX,\by)(\bx)} \le \SURRERR(\bx,n,\mX,\by) \qquad \forall f \in \calc.
\end{gather}
\end{subequations}
...
\begin{gather} 
    \MINVAL(\bx,n,\mX,\by) = \APPMIN(n,\mX,\by) - [f(\bx) - \SURRERR(\bx,n,\mX,\by)], \\
    \bx_{n+1} = \argmax_{\bx \in \Omega} \MINVAL(\bx,n,\mX,\by).
\end{gather}
\end{subequations}
