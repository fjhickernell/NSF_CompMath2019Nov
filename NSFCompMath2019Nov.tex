...
We then evaluate the residuals at the original samples, $\br:=\by-\by_T$ and fit our variation model to these residuals $\SVAR(n,\mX,\br)$.
...
The trend function $f_{LS}(\bx)$ is a quasi-interpolant because it does not exactly agree with the function values at each data point. The residual at the data points $r_i = f_i - f_{LS}(\bx_i)$ will be represented by a Gaussian process kriging model $r_{K}(\bx)$.
...
The trend model minimizes the residual $\sum w_i(\bx)(f_i - f_{LS}(\bx_i))^2$, where the weights, $w_i(\bx)$ depend on the location where the function will be evaluated. In the Bayesian bootstrap method \cite{rubin1981bayesian, efron1986bootstrap, efron2016computer} we generate an ensemble of trend models, $f_{LS}^b$, $b=1,B$. Each trend model is defined by minimizing $\sum v_i^b w_i(\bx)(f_i - f_{LS}(\bx_i))^2$.   The reweighting factors, $v_i^b$, are the $i^{th}$ component of the $b=1,B$ sample $\bv^b$ that are uniformly distributed over the $n$ dimensional unit simplex, $\sum v_i^b = 1 $.  

After each trend model is defined, we create the corresponding variation model and evaluate uncertainty at each of the candidate sample locations.  This data is then used as input for the Acquisition Function to identify what candidate sample is most likely to reduce the uncertainty in our QOIs. 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\notes{Mac: The subsections on important coordinate directions need to go to section 3}
...
\item Estimating the variation $\SVAR_b(n,\mX,\by - \by_T^b)$ on the residuals, $\by - \by_T^b$ with its uncertainty $\SURRERR_b(n,\mX,\by)$
