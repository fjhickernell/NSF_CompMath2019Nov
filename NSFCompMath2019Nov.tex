...
The trend MLS model is evaluated at the original set of samples to define $\by_{\TREND} = \{ \STREND(n,\mX,\by)(\bx_i) \}_{i=1}^n$.
We then evaluate the residuals at the original samples, $\br:=\by-\by_{\TREND}$ and fit our variation model to these residuals $\SVAR(n,\mX,\br)$.
...
The trend MLS model is evaluated at all of  the candidate samples to define  $\by^c_{\TREND} = \{ \STREND(n,\mX,\by)(\bx^c_i) \}_{i=1}^n$.
The variation model then used to estimate the uncertainty at each of the candidate sample points.  
\FJHNote{I think that the $\mX^c$ needs to be input into $\VAL(\cdot, n, \mX, \by)$.}

The above process is repeated using weighted Bayesian bootstrap resampling to create an ensemble of trend models by refit the MLS with random weights.
...
The distributions for the uncertainties at the candidate points are then used to define the acquisition function $\VAL$ to identify which of the candidate sample points most reduce the uncertainty in the predicted $\APP$. 
...
The goal of the initial exploratory samples is to explore the space of feasible POIs, $\Omega$,  when little, or nothing, is known about the response QOIs. 
 We use  Sobol' points or other low discrepancy (space filling) samples \cite{DicPil10a} in $\Omega$, 
  and  will compare the effectiveness of low discrepancy samples from either uniform and Chebyshev distributions.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Global surrogate models based on global parametric trends of the data can only capture multivariate response surfaces 
