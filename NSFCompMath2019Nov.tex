...
\begin{align}
\label{eq:iderr}
     \bignorm[\calg]{f - \APPID(n,\mX,\by)} &\le \norm[\calg]{\SURRERR(n,\mX,\by)} =: \IDERR(n,\mX,\by), \\
     \label{eq:minerr}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsubsection{RKHS or Kriging Surrogate Models} \label{sec:MinPredErr}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Recall our two key QOIs in \eqref{eq:ourQOIs}, their approximations in \eqref{eq:QOIhat}, and the acquisition functions in \eqref{eq:QOIval}. A common  acquisition function for function approximation correspond to the prediction error bound of the surrogate in \eqref{RKHSErrBd}:
...
Noting that $\bignorm[\calf]{\SURR(n,\mX,\by)} = \sqrt{\by \mK^{-1} \by}$ error bound \eqref{RKHSErrBd} then implies an error bound that depends completely on output data: 
...
There are different ways to overcome this curse, all of which boil down to defining an $\calc$ of reasonable functions that have an essentially low-dimensional structure.  \textbf{stopping here}
