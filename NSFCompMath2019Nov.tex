...
\end{equation}
Our problem is to numerically approximate these QOIs when $d \in [10, 100]$ and the feasible number of simulations, $n$, is relatively small, say in the hundreds.  The primary cost of our algorithms will be the cost of running the simulations to obtain $f(\bx_1), \ldots, f(\bx_n)$.
...
To approximate or optimize a $d$-variate function using $n$ function data, where $d$ is in the dozens, and $n$ is restricted to the thousands, the function $f$ cannot be fully $d$ dimensional.  For example, even a multilinear function,
\begin{equation*}
    f(\bx) = a_0 + a_1 x_1 + a_2 x_2 + \cdots + a_{1,2} x_1 x_2 + \cdots + a_{1,\ldots, d}x_1 \cdots x_d, \qquad \bx \in [-1,1]^d,
\end{equation*}
requires at least $n = 2^d$ function data to approximate or optimize well.  Exponential growth in the sample size required for a reasonable solution is called the \emph{curse of dimensionality}.
