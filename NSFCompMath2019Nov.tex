...
The initial design must be fill the domain well enough to detect significant fluctuations in the function.
\end{keyidea}
...
\begin{keyidea} \label{keyideaBayesian}
In addition to a deterministic numerical analysis, one may employ a Bayesian numerical analysis.  The two perspectives are similar.  The latter allows one to perform inference.
\end{keyidea} 
...
Adaptive approximation algorithms are constructed for cones of functions that can be roughly approximated via a modest number of observations.
\end{keyidea}
...
Let $\calf$, be a finite dimensional Banach space of multi-linear functions:
\begin{equation}
    \calf = \left\{f:[0,1]^d \to \reals : f = \sum_{\fraku \subseteq 1:d} \hf_{\fraku} \prod_{j \in \fraku}x_j \right \}, \qquad \norm[\calf]{f} : = \norm[\infty]{(\hf_\fraku/\gamma_\fraku)_{\fraku \subseteq 1:d}}
\end{equation}
for some prescribed set of weights, $(\gamma_\fraku)_{\fraku \subseteq 1:d}$, where $1:d := \{1, \ldots, d\}$.  For the sake of simplicity, suppose that you are able to sample the coefficients, $\hf_\fraku$, so in this case, $\APP(f,n) = \sum_{i=1}^n \hf_{\fraku_i} \prod_{j \in \fraku_i}x_j$, and the design is $\{\fraku_1, \ldots, \fraku_n\}$.
