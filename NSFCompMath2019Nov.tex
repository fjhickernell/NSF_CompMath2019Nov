...
\begin{center}
\Large \textbf{Adaptive Sampling for Multivariate Problems\\ 
...
Each large-scale simulation for grand-challenge problems, such as modeling global climate change, can require hours or even days of high performance computation.   The quantities of interest (QOIs) depend on $d \in [10, 100]$ input parameters of interest (POIs), denoted $\bx$.  The model output or response can be viewed as a map $f:\Omega \subseteq \reals^d \rightarrow \reals$.   Possible QOIs include function approximation, optimization, and integration:
\begin{equation}
...
\end{itemize}   
Multivariate surrogate models and emulators typically underlie $\APP$.

Limited computer resources and the curse of dimensionality prevent acquiring a dense sample $\mX_{1:n}$, in the input space. Therefore, it is important to sample adaptively, choosing the next data site, $\bx_{n+1}$, in terms of $\mX_{1:n}$ and  $\by_{1:n}$.  This is typically accomplished via
\begin{equation} \label{eq:nextsample}
    \bx_{n+1} = \argmax_{\bx \in \Omega} \VAL(\bx,n,\mX_{1:n}, \by_{1:n}),
\end{equation}
where $\VAL(\bx,n,\mX_{1:n}, \by_{1:n})$ measures the value to $\APP(n+1,\cdot,\cdot)$ in observing $f(\bx)$, given what has been observed already.  Such $\VAL$ are sometimes called acquisition functions and are typically based on a measure of uncertainty in the approximate QOI so far, i.e., $\APP(n,\mX_{1:n},\by_{1:n})$.
...
The PIs, Yuhan Ding (YD), Fred J Hickernell (FJH) and Mac Hyman (MH), propose to find better $\APP$ and $\VAL$ than what is current best practice.  Current $\VAL$ do not assess the uncertainty of $\APP$ as well as they should, often because they do not take full account of $\by_{1:n}$.

We also plan to derive error data-driven bounds satisfying
\begin{equation} \label{eq:errbd}
    \bignorm[\calg]{\QOI(f) - \APP(n,\mX_{1:n},\by_{1:n})} \le \ERR(n,\mX_{1:n},\by_{1:n}) \qquad \forall f \in \calc,
\end{equation}
where $\calc$ is a precisely defined set of maps between POIs and outputs.  This will allow the user to decide whether to additional data is needed to improve the accuracy of $\APP(n,\mX_{1:n},\by_{1:n})$.  Most current $\ERR$ are based on heuristics with no rigorous understanding of what $\calc$ should be. 

\subsection{Our plan}

Our plan follows the popular two-step \emph{explore-exploit} process.  In the \emph{explore} step, little or nothing is known of the QOIs or of $f$.  We sample $f$ on a relatively sparse, but space-filling, set of $n_0$ data sites over the region of feasible POIs.  If $f$ is detected to be highly variable in one portion of the domain, $\Omega$, then more data sites should be placed there during the next step. This initial design must be large enough to detect important features of $f$ that influence $\QOI(f)$, but small enough to be economical. The initial design does not depend on function data.

Starting with $n = n_0$, we then \emph{exploit} the existing sample to create multiple approximations $\APP_{\btheta}(n, \mX_{1:n}, \by_{1:n})$ for $\btheta \in \Theta$.  These may be involve
\begin{itemize}
\item weighted resampling of the data, as in a bootstrap-like approach, where $\btheta$ represents different weights (Section \ref{sec:Bootstrap}),
...
Applications include:\\
 cubature - \\
 optimization - this approach can guide an algorithm to identify a good starting location for a gradient descent algorithm,\\
