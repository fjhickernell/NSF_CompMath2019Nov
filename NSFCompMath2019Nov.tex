...
\usepackage{latexsym,amsfonts,amsmath,amsthm,epsfig,extdash,mathabx,multirow}
...
The surrogate models and uncertainties also provide us uncertainties or error bounds for the QOIs, which in general, take the form
...
\paragraph*{Summary} 
Our goals are to create better surrogate models, $\SURR$, and data-driven surrogate uncertainty measures, $\SURRERR$, especially in the case when $d$ is large and $n$ is small.  These will require more flexible and data-driven models and correspondingly economical adaptive sampling schemes.  The end result will be
\begin{itemize}
    \item A methodology to combine the data-driven Bayesian bootstrap analysis for the surrogate trend model with Gaussian process variational modeling to quantify the overall uncertainty in the surrogate model for the $\APP$,
...
In this section we provide more details and relevant notation to describe adaptive sampling from our perspective.  The initial sampling budget must be devoted to exploring $f$ over its domain, $\Omega$.  Without a wide-ranging exploration, important features might be missed.  Of course, if there is a priori knowledge about the output map $f$, that can be built into the exploratory stage, but we will ignore that factor.  After the exploration comes the exploitation stage, where the simulation output or function data, $\by$ is used to determine where future samples should be taken.

A key feature of the exploitation stage is the surrogate model (metamodel, emulator, function approximation), that is base only on the data, $\mX$ representing the data sites, and $\by$ representing the output data.  In both the RKHS and kriging approaches to constructing surrogate models, there is a trend and a variation, an approach we adopt here:  
\begin{equation*}
    \SURR(n,\mX,\by) = \STREND(n,\mX,\by) + \SVAR(n,\mX,\by). 
\end{equation*} 
The trend, $\STREND(n,\mX,\by)$ is relatively simple function that fits the data roughly.  Rather than using a low degree polynomial, we use moving least squares (MLS) for to model the trend (Sect.\ \ref{sec:trend}).  The variation, $\SVAR(n,\mX,\by)$, models the difference between the function and the trend and provides interpolation.  We use reproducing/covariance kernels to construct the variation model (Sect.\ \ref{sec:varmodel}).  

The surrogate model for $f$ requires a measure of uncertainty, which we construct using uncertainty measures for $\SVAR(n,\mX,\by)$ in Sect.\ \ref{sec:varmodel} combined with a Bayesian bootstrap resample in Sect.\ \ref{sec:BayesianBootUncertainty}. We then exploit the surrogate model and its uncertainty to define the acquisition function, $\VAL(\cdot, n,\mX,\by)$  which guides as to where to sample next.  This is addressed in Sect.\ \ref{sec:acquire}.  In lieu of an exact maximization we evaluate  $\VAL(\cdot, n,\mX,\by)$ at a dense set of $n_c \gg n$ space filling points.
...
\subsection{The Initial Exploration Step: Space Filling Sampling}
...
We define our trend model as the MLS function,$\STREND(n,\mX,\by)(\bx) = f_{\LS}(\bx)$, by minimizing  $\sum w_i(\bx) (f_i - f_{\LS}(\bx_i))^2$ over the available sample points.  The weights decrease as a function of distance from the location, $\bx$, where the function $\hat f$ will be evaluated.   We will initially use a Gaussian kernel weight,  $w_i(\bx) = e^{(\bx-\bx_i)^2/\sigma(x)^2}$.  
...
\begin{wrapfigure}{r}{0.5\textwidth}
	...
	\label{MLS}}
\end{wrapfigure}
this additional computation is minimal compared to the expense of obtaining the sample values.  

  
In $d$ dimensions, the number of basis functions, $K_p$,  for a polynomial of degree $K_d$ grows exponentially as $K_p={K_d + d \choose K_d} -1$ and  will far exceed the number of samples for even modest degree polynomials in high dimensions.   
We will typically use a MLS quadratic basis functions for low dimensional problems $d<10$, and linear basis functions for high-dimensional problems.    
While global interpolants are very `stiff', these MLS are flexible and can fit odd surfaces similar to how splines can approximate nonpolynomial shaped data in one dimension. 
Figure \ref{MLS} illustrates the flexibility of a  MLS with linear basis function fitting scattered data in two dimensions.  


After fitting the sample data, we will also generate a set of $n_c>>n$ low discrepancy uniformly distributed samples $\mX^c = \{ \bx^c \}_{i=1}^{n_c}$ in $\Omega$ to be 
considered for the next input POIs.  The MLS trend function $f_{\LS}$ is evaluated at each of these candidate points to create a trend approximation $\by^c = \{ f_{\LS}(x^c_i \}_{i=1}^{n_c}$ for the response. 
 
The trend function $f_{\LS}(\bx)$ is a quasi-interpolant because it does not exactly agree with the function values at each data point. We will then use the residuals at the data points $r_i = f_i - f_{\LS}(\bx_i)$ to create a model for variation in the trend model. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Following the Bayesian bootstrap method \cite{rubin1981bayesian, efron1986bootstrap, efron2016computer}, we generate an ensemble of $B$ trend models, 
\begin{equation}
    \STREND^b(n,\mX,\by)(\bx) = f^b_{\LS}(\bx), \qquad \text{where } f^b_{\LS} = \argmin_{g \in \calp} \sum_{i=1}^n v_i^b w_i(\bx)(y_i - g(\bx_i))^2,
\end{equation}
where as in Sect.\ \ref{sec:trend}, the $w_i$ are the MLS weight functionss, and now $\bv^b$ is an $n$-vector uniformly distributed over the unit simplex, $\cals^n : = \{\bv \in [0,1]^n : v_1 + \cdots + v_n = 1\}$.  
...
\end{equation*}
and the uncertainty surrogate model defined as
\begin{equation*}
    \SURRERR(n,\mX,\by) = \sqrt{\frac 1B \sum_{b=1}^B \bigl[ \SURR^b(n,\mX,\by) + \SVARERR^b(n,\mX,\by - \by^b_{\TREND}) - \SURR(n,\mX,\by) \bigr]^2} .
\end{equation*}


\iffalse
, as in Sect.\ \ref{sec:varmodel},  After each trend model is defined, we create the corresponding variation model and evaluate uncertainty at each of the candidate sample locations.  This data is then used as input for the Acquisition Function to identify what candidate sample is most likely to reduce the uncertainty in our QOIs. 


On every weighted bootstrap fit of the data, we define a surrogate model $\SURR_b$ by 
\begin{enumerate}
\item Fitting a trend model $\STREND_b(n,\mX,\by)$ to the available samples that minimizes $\sum v^b_iw_i(\bx)(f_i - f_{\LS}(\bx_i))^2$ 
\item Estimating the variation $\SVAR_b(n,\mX,\by - \by_{\TREND}^b)$ on the residuals, $\by - \by_{\TREND}^b$ with its uncertainty $\SURRERR_b(n,\mX,\by)$
\item Use the surrogate model, $\SURR_b=\STREND_b + \SVAR_b$, to quantify the uncertainty for the bootstrap surrogate model at each of the candidate points. 
\end{enumerate}
We then $\SURR_b=\STREND_b + \SVAR_b$ these ensemble of uncertainties  at $\bx^c$ to define the \VAR at the candidate points. 
\fi


\subsection{The Acquisition Step:  Uncertainty Quantification} \label{sec:acquire}
\FJHNote{I will write something}
...
\end{align}
\end{subequations}
The next data site, $\bx_{n+1}$ maximizes the acquisition function (see \eqref{eq:nextsample}).  
...
Once the best candidate sample is selected, then additional candidate samples can be generated nearby this location and the process iteratively repeated.  Alternatively, the best candidate could be used as a starting location for a numerical optimization algorithm to maximize $\VAL(\bx)$ for the next sample location.  We will consider both of these approaches. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Global surrogate models based can only capture the trends trends in a very limited class  multivariate response surfaces.  For example, in one-dimensional problems low-degree polynomials are able to capture simple trends in the data, but fail to accurately represent nonpolynomial shapes, such as Gaussian kernels or step functions.  Piecewise polynomial interpolants, such as cubic Hermite polynomials, can effectively represent these surfaces, but don't generalize easily to very high dimensions with scattered sample data.  

The MLS quasi-interpolants are an effective alternative to piecewise polynomials in that the approximations are able to capture global nonpolynomial behavior.  The quasi-interpolant depends more on the nearby points than the more distant ones.  One of the challenges in MLS is to identify a robust methodology for picking the appropriate  length scale, $\sigma(\bx)$, for the MLS kernel.  

Our current approach is to use the mean distance to the nearest $K_n$ neighbors of $\bx$, where $K_n$ is greater than the number of basis functions, $K_p$  in the local approximating function is based on empirical experiments.  
...
\FJHNote{I will re-write.  Moving stuff here for now from other places.}

Because the identity map is homogeneous, i.e., $\ID(cf) = c\ID(f)$ for all $c \in \reals$, our surrogate models, $\SURR$, and surrogate uncertainties, $\SURRERR$, are also constructed to be homogeneous.  It then follows that the sets of functions, $\calc$, for which the error bounds \eqref{eq:surrUncert} are guaranteed, are \emph{cones}, i.e., $f \in \calc \implies cf \in \calc$.
