...
Each large-scale simulation for grand-challenge problems, such as modeling global climate change, can require hours or even days of high performance computation.   The output quantities of interest (QOIs) often depend on $d \in [10, 100]$ input parameters of interest (POIs), denoted $\bx$.  The model output or response can be viewed as a map $f:\Omega \subseteq \reals^d \rightarrow \reals$.   Possible QOIs include function approximation of surrogate models for global uncertainty analysis, exploring nonconvex response surfaces in numerical optimization, and the integration of multivariate high-dimensional functions:
\begin{equation}
    \QOI(f) = f, \quad \QOI(f) = \min_{\bx \in \Omega} f(\bx), \quad \QOI(f) = \argmin_{\bx \in \Omega} f(\bx), \quad \QOI(f) = \int_{\Omega} f(\bx) \, \dif \bx.
\end{equation}

Numerical algorithms for approximating these QOIs take the form $\APP(n,\mX_{1:n},\by_{1:n})$, where 
\begin{itemize}
    \item $\mX_{1:n} := (\bx_1, \ldots, \bx_n)^T \in \Omega^{n} \subseteq \reals^{n \times d}$ is a well chosen design, i.e., an array of POI values or data sites, and
    
    \item $\by_{1:n} := (f(\bx_1), \ldots, f(\bx_n))^T$ is (noiseless) function data, i.e., a vector of outputs.
\end{itemize}   
The $\APP$s can be investigated between the samples using multivariate surrogate models or emulators.

Limited computer resources and the curse of dimensionality prevent acquiring a dense sample $\mX_{1:n}$, in the input space. Adaptive sampling  uses past samples as a guide in choosing future samples to maximize the expected information.   That is, the next sample, $\bx_{n+1}$, is a function of both $\mX_{1:n}$ and  $\by_{1:n}$ and chosen to maximize the expected value it will bring to the $\APP$s, 
\begin{equation} \label{eq:nextsample}
    \bx_{n+1} = \argmax_{\bx \in \Omega} \VAL(\bx,n,\mX_{1:n}, \by_{1:n})~.
\end{equation}
Here $\VAL(\bx,n,\mX_{1:n}, \by_{1:n})$ measures the value to $\APP(n+1,\cdot,\cdot)$ in observing $f(\bx)$, based on what has been already observed.  The $\VAL$ function is sometimes called an acquisition functions and is typically based on a measure of uncertainty in the approximate QOI so far, i.e., $\APP(n,\mX_{1:n},\by_{1:n})$.
...
The PIs, Yuhan Ding (YD), Fred J Hickernell (FJH) and Mac Hyman (MH), propose to find better surrogate models, $\APP$s,  and adaptive sampling acquisition functions, $\VAL$, than are currently available.  For example, many $\VAL$ functions only use the past sample locations $\mX_{1:n}$ to assess the uncertainty of $\APP$ and do not take into account the sample values $\by_{1:n}$.

Our algorithms will be guided by deriving error data-driven bounds of the form 
\begin{equation} \label{eq:errbd}
    \bignorm[\calg]{\QOI(f) - \APP(n,\mX_{1:n},\by_{1:n})} \le \ERR(n,\mX_{1:n},\by_{1:n}) \qquad \forall f \in \calc.
\end{equation}
That is, we will derive estimates for the error, $\ERR$, based on a precisely defined set of maps, $\calc$, between the input POIs and output QOIs.  This will allow the user to decide whether to additional data is needed to improve the accuracy of $\APP(n,\mX_{1:n},\by_{1:n})$.  Most current $\ERR$ are based on heuristics with no rigorous understanding of what $\calc$ should be. 
...
The goal of the \emph{exploit} stage is to detect regions where $f$ is poorly represented and place more future samples to reduce the uncertainty. 
Starting with $n = n_0$, we then \emph{exploit} the existing sample to create multiple approximations $\APP_{\btheta}(n, \mX_{1:n}, \by_{1:n})$ for $\btheta \in \Theta$.  
...
\end{enumerate}
When the QOI is function approximation or optimization, then $\APP_{\btheta}(n, \mX_{1:n}, \by_{1:n})$ will the approximation of $f$.  When the QOI is the integral may be approximated directly or may be the integral of a function approximation. Because the primary time cost in our problems is acquiring data, we are not overly concerned about the time required to all of the $\APP_{\btheta}(n, \mX_{1:n}, \by_{1:n})$ needed.

The $\APP_{\btheta}(n_0, \mX_{1:n}, \by_{1:n})$ are then used to choose 
\begin{itemize} 
\item a best approximation, $\APP(n, \mX_{1:n}, \by_{1:n})$,

\item a measure of the uncertainty of our approximation, $\ERR(n,\mX_{1:n}, \by_{1:n})$ for a well defined set of functions $\calc$, as in \eqref{eq:errbd},  and 

\item $\VAL(\bx,n,\mX_{1:n}, \by_{1:n})$, which tells us where to choose the next data site, $\bx_{n+1}$, via \eqref{eq:nextsample}.
\end{itemize}
This exploit step is repeated until the time budget is exhausted or the uncertainty tolerance is met.

For $\QOI(f) = f$, $\VAL(\bx,n,\mX_{1:n}, \by_{1:n})$ will correspond to a pointwise determinstic error bound or probabilistic error standard error.  For $\QOI(f) = \min_{\bx \in \Omega} f(\bx)$, $\VAL(\bx,n,\mX_{1:n}, \by_{1:n})$ may be determined from sensitivity analysis or correspond to expected improvement.  Because function approximation, optimization, and integration are all homogeneous problems, i.e., $\QOI(cf) = c\QOI(f)$ for all $c \in \reals$, our algorithms and error bounds are also constructed to be homogeneous.  It then makes sense that our sets of functions $\calc$, for which the error bounds are guaranteed, are \emph{cones}, i.e., $f \in \calc \implies cf \in \calc$.

\subsection{Our impact}
\paragraph{\bf Theoretical advances}
The theoretical advances for error estimates will extend mathematical foundation for adaptive sampling by 
\begin{itemize}
\item Providing error estimates based on both the sample locations and function values,
\item Extending the theoretical basis for ...
\end{itemize}

 \paragraph{\bf Algorithms}
Our algorithms will have an impact on applications where function evaluations are expensive by
\begin{itemize}
\item \underline{function approximation:} creating a surrogate model, with uncertainty estimates, to investigate the response of the QOIs over the entire feasible range of POIs using a minimal number of samples.  This model can be used for global sensitivity analysis of dynamic (interactive) visualization of the response surface. 
\item \underline{optimization:} The surrogate model can identify multiple locally optimal solutions of a response function.  The model will provide a large scale, global, representation of the response function that can be used to identify a good starting locations for gradient descent algorithms.
 \item \underline{integration:} The surrogate model can be used as a control variate to detrend the data and significantly decrease the numerical integration error of functions defined on a sparse, arbitrary spaced, grid \cite{}.  
\end{itemize}

\paragraph{\bf Software}
We will provide software so our algorithms will be useful and used by the modeling community.  The software will include 
\begin{itemize}
\item adaptive samplers
\item surrogate models
\item application software for function approximation, optimization, and integration
\end{itemize}
\paragraph{\bf Broader Impact}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
