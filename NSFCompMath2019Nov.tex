...
\newcommand{\DWLS}{\textrm{DWLS}\xspace} %distance weighted least squares, also known as moving least squares
...
\FJHNote{Here are some thoughts.  May we define our surrogate as the sum of a trend and a variation, i.e., $\SURR(n,\mX,\by) = \STREND(n,\mX,\by) + \SVAR(n,\mX,\by)$. Then we will estimate $\STREND(n,\mX,\by)$ by \DWLS and $\SVAR(n,\mX,\by)$ by kriging.  We will initially propose a two-step process to get the surrogate model, but then later we may try to combine.  

Now the question I still have, is how will your Bayesian bootstrap be used in your \DWLS, since I do not see how it's uncertainty is needed.  I do like \DWLS approach because it gives us more flexibility than a polynomial, and I think that we should mention that.  Also, since \DWLS is a quai-interpolant, we can get a surrogate model that interpolates when we add in the $\SVAR(n,\mX,\by)$ term.   }
