...
\item[Smolyak Algorithms \cite{BunGrie04a}] based on sparse grids have the weaknesses mentioned above for sparse grids.  Moreover, any algorithm based on a grid will have challenges if we want to rotate the coordinate system to identify a low dimensional structure.
\item[Continuous Tensor Trains \cite{GorKarMar19a}] are used by Chebfun \cite{DriHalTre14a} fr low dimensional multivariate function approximation, but do not scale well to high dimensions.
\end{description}
