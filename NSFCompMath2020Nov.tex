...
The output \hypertarget{QOIlink}{quantities of interest} (\QOIs) might be an inexpensive representation of the simulation output (the identity) or the optimal output:\footnote{Other \QOIs include the location of the optimum output and (parametric) integrals (averages) of the output.} 
...
In these cases, we may only have a modest number, $n$, of expensive function values, such as output from large-scale simulation runs or real-world experiments. 
...
\item $\mX := (\bx_1, \ldots, \bx_n)^T \in \Omega^{n} \subseteq \reals^{n \times d}$ is a set of input \hyperlink{POIlink}{POI\xspace} vectors, also known as data sites or sampling location.  These can include model parameters and the initial conditions for a simulation.  The goal is to adaptively identify a well chosen \emph{experimental design} for the sampling locations. 
\item $f$ is function, such as a large-scale computer simulation, which generates the information required for the \QOIs. 
\item $\by := \bigl(f(\bx_1), \ldots, f(\bx_n) \bigr)^T \in \reals^n$ is a vector of (noiseless\footnote{Noisy data is an important case for future study.}) \emph{output function data}.
...
The \emph{exploitation} stage uses all existing samples to construct surrogate models, $\SURR(n,\mX,\by)$, for $f$, together with their uncertainties, $\SURRERR(n,\mX,\by)$, so that we may detect regions where $f$ is poorly represented with respect to its influence on $\APP(n,\mX,\by)$. The uncertainties in the surrogate models inform the acquisition functions, which sequentially determine each new sample location.
...
The trend model can be based on a simple approximation for $f$, such as a reduced model or a global multivariate polynomial.
%in low dimensions $(d<5)$.  
The number of terms in complete multivariate polynomials grows exponentially as $d$ increases. 
We use a variation of the moving least squares (\MLS) method \cite{liumovingpartI1997, limovingpartII1996, salehi2013generalized, mederos2003moving,cheng2008survey} to improve the flexibility of lower degree polynomials
...
The residuals or lack of fit to the data, $\by_{\VAR} := \by - \by_{\TREND}$, where $\by_{\TREND} = \bigl( \STREND(n,\mX,\by)(\bx_i) \bigr)_{i=1}^n$, are then  fit by the variation model, $\SVAR(n,\mX,\by_{\VAR})$, a Gaussian process kriging model or reproducing kernel Hilbert space (RKHS) minimum norm interpolant (see Sect.\ \ref{sec:varmodel}). The Gaussian process and RKHS perspectives yield essentially equivalent outcomes. We infer the parameters in our covariance/reproducing kernel to obtain better accuracy under a stringent sample budget (Sect.\ \ref{sec:kerinferdata}). 

We assess the uncertainty in our surrogate models using \emph{data-based measures}, $\SURRERR(n,\mX,\by)$, for the output maps in \emph{candidate sets}, $\calc$, such that 
\begin{equation} \label{eq:surrUncert}
\abs{f(\bx)-\SURR(n,\mX,\by)(\bx)} \le \SURRERR(n,\mX,\by)(\bx) \qquad \forall f \in \calc,\ \bx \in \Omega,
\end{equation} 
either absolutely or with high probability. Our measure of uncertainty combines the uncertainty in both the trend and variation models (Sects.\  \ref{sec:uncertVar} and \ref{sec:BayesianBootUncertainty}).

\subsection{Surrogate Models and Uncertainties Drive Adaptive Sampling} 
If the \QOIs in \eqref{eq:ourQOIs} correspond to the function itself or its minimum, then the corresponding approximations are
\begin{equation} \label{eq:QOIhat}
\APPID(n,\mX,\by) = \SURR(n,\mX,\by), \qquad \text{or}\qquad \APPMIN(n,\mX,\by) = \min_{1 \le i \le n} y_i,
\end{equation}
respectively. For $\APPID$ the surrogate model needs to represent $f$ well throughout its domain, $\Omega$, while for $\APPMIN$ the surrogate model only needs to represent $f$ well enough near potential minima.
...
This function, $\VAL$, relies on a pointwise error bound for the surrogate model and depends on both the surrogate and  uncertainty models.  For example, for function minimization, $\VAL$ corresponds to the expected improvement in the approximation to the minimum. The precise definitions are given in Sect.\ \ref{sec:acquire}.

The surrogate models for $f$ lead to \emph{error-driven bounds} on the uncertainty in the $\APP$ models:
...
Our proposed research will demonstrate that these error bounds are valid for $\calc$, precisely defined candidate sets of well-behaved simulation output maps, $f$. 
The exploit stage repeats until the time budget is exhausted or until  the uncertainty in the QOI, $\ERR(n,\mX,\by)$, is no greater than the tolerance, $\varepsilon$.
...
For \eqref{eq:errbd}  to hold, any two functions in $\calc$ that share the same data must have similar \QOIs.  We will require that for all $f, g \in \calc \text{ with } f(\mX) = g(\mX) = \by$, the difference between the \QOIs satisfy a relationship such as 
...
This is disastrous for large $d$.  
To mitigate this curse we choose $\calc$ to only include functions with \emph{low effective dimension}, $d_\eff < d$, which our algorithm can identify and where $\ERR(n,\mX,\by) = \Order(n^{-r/d_\eff})$.  This can be done via coordinate weights or active subspaces (Sect.\ \ref{sec:kerinferdata}). 

Choosing  $\calf$ to have a fixed norm does not allow sufficient flexibility, since we may not know beforehand which normed space best fits our input function, $f$.  Thus, our $\calc$ will  in fact be a subset of $\cup_{\btheta \in \Theta} \calf_\btheta$, a \emph{union} of parameterized spaces, $\calf_\btheta$, each with its associated norm.  A research problem will be choosing $\{\calf_\btheta : \btheta \in \Theta\}$ large enough to allow flexibility, while ensuring that an appropriate $\btheta$ can be inferred from a reasonable amount of function data (Sect. \ref{sec:kerinferdata}).
...
\item Better adaptive sampling acquisition functions, $\VAL$, which depend on both the past sample locations, $\mX$, and the output data, $\by$, (Sect.\ \ref{sec:acquire}),
...
Since quantifying the uncertainty in our surrogate model is only expected to require seconds on a laptop computer, this cost is insignificant compared to the cost of acquiring the sample data when analyzing large-scale simulations.   
...
\caption{Uniform IID data sites (left plot) are  contrasted with space filling designs mimicking the low discrepancy uniform  and Chebyshev distributions. Notice the difference between the clustering of points in the IID uniform sampling and regular distribution of points in the low discrepancy uniform sampling. 
We will use low discrepancy space-filling sampling in our exploration stage.
\label{PtsFig}}
\end{wrapfigure}

The goal of the initial samples is to explore the space of feasible \POIs, $\Omega$, when little, or nothing, is known about the response \QOIs. We use space-filling (low-discrepancy) designs such as Sobol' points \cite{DicPil10a} or integration lattices \cite{SloJoe94, DicEtal14a} in $\Omega$.  Fig.\ \ref{PtsFig} illustrates how IID uniform sampling is more likely to over-, or under-, sample some regions than a low discrepancy (Sobol) sample.

Interpolating between sample locations is more reliable than extrapolating outside their convex hull.  The Chebyshev (arcsine) distribution pushes points towards the boundary of $\Omega$, and the convex hull of the resulting distribution of samples includes more of $\Omega$ than uniform sampling. (In general, we cannot afford to sample on all $2^d$ corners of the domain.) We will compare the effectiveness of low discrepancy sampling for Chebyshev and uniform distributions to see which approach is the most effective in defining the surrogate model. 
 The advantage of doing so may depend on the smoothness of $f$ \cite{HicLi12a}.
For simplicity, our initial studies will assume hyper-rectangular domains, $\Omega$, as pictured in Fig.\ \ref{PtsFig}.  
...
The exploratory set of $n_0$  sites does not depend on $f$ or the \QOIs.  It must be small enough to be economical, but large enough to detect important features of $f$ that influence $\QOI(f)$. In some applications, one may have access to inexpensive function values or data collected from earlier experiments.  Our initial exploration stage will include this data as well.
The large-scale simulation is run for each POI vector to obtain $f(\bx_1), \ldots, f(\bx_{n_0})$, and then we begin the exploitation stage.
...
\end{equation}
and $\calp$ is a finite dimensional vector space, e.g., low degree polynomials. The weights $w_i(\bx)$ decrease with distance $\norm[2]{\bx - \bx_i}$. We will initially use a Gaussian kernel weight, $w_i(\bx) = \me^{-\norm[2]{\bx - \bx_i}^2/\sigma(\bx)^2}$, where $\sigma(\bx)$ is the mean distance from the evaluation point, $\bx$, to a set of $\kappa_n$ nearest neighbors \cite{cheng2008survey}. The number of nearest neighbors will be greater than $\dim(\calp)$.
...
\label{MLS}}
\end{wrapfigure}
Every time the MLS interpolant is evaluated, we solve a least squares system that weights the nearby points much more than the distant points. For our problems, this additional
computation is minimal compared to the expense of obtaining the sample values. Note that the trend model exactly reproduces any $f \in \calp$.
...
Global low-degree polynomial approximations are `stiff' and cannot bend to follow irregular surfaces.  

The MLS approximations are based on local data; Even low-degree polynomials are flexible, similar to the way splines fit data in one dimension.
We will use quadratic polynomials for $\calp$ for low dimensional problems $d<10$, and linear polynomials for high-dimensional problems. 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The trend model, $\STREND(n,\mX,\by)$, is a quasi-interpolant because it does not exactly fit the data. Having modeled the general, we now model its variation, $f_{\VAR} = f - \STREND(n,\mX,\by)$, using a minimum norm RKHS interpolant. Suppose that $f_{\VAR}$, belongs to an RKHS $\calf$. A commonly used reproducing kernel is the squared exponential (or Gaussian) kernel:
\begin{equation} \label{eq:GaussKer}
K(\bt,\bx) =  \exp(-\norm[2]{\btheta \odot (\bt-\bx)}^2),
\end{equation}
where $\odot$ denotes the term-by-term (Hadamard) product, and $\btheta \in (0, \infty)^d$ is a parameter to be set or inferred. See \cite{Buh00, Fas07a, FasMcC15a, ForFly15a, ForEtal09, SchWen06a, Wen05a} for an explanation of function approximation in RKHSs. 

The minimum norm interpolant of $f_{\VAR}$ in this $\calf$ using the residual data, $
...
The deterministic framework using RKHSs for $f_{\VAR}$ has a parallel Bayesian or probabilistic numerics interpretation promoted by Diaconis and others \cite{BriEtal18a, Dia88a, OHa91a, OwhEtal19a, RasWil06a, Rit00a}. It assumes that $f_{\VAR}$ is an instance of $\GP(0,K)$, a Gaussian process with mean zero and covariance kernel, $K$. In this case the posterior mean of $f_{\VAR}(x)$ given the data $(\mX,\by)$ is called a kriging model and has the same expression as $\SVAR(n,\mX,\by)$ as in \eqref{appxExOne}.  The width of the pointwise credible interval for the surrogate variation model is the same as $\SVARERR(n,\mX,\by)$ in \eqref{eq:DataErrBd} below, with $A(\mX)$ determined by empirical Bayes and dependent only on $n$. We will adopt both deterministic and Bayesian approaches, depending on which better suits our needs.
...
Along with the surrogate model, $\SURR(n,\mX,\by)$, we must construct a data-based pointwise error bound or measure of its uncertainty, $\SURRERR(n,\mX,\by)$ (see \eqref{eq:surrUncert}).  That requires translating the error bound in \eqref{eq:RKHSErrBd} into a quantity that does not require a priori the knowledge of $\norm[\calf]{f_{\VAR} - \SVAR(n,\mX,\by_{\VAR})}$.  Our approach is discussed in Sects.\ \ref{sec:uncertVar} and \ref{sec:BayesianBootUncertainty}.

Given the surrogate model and its uncertainty, we may then define the acquisition functions for function and minimum approximation respectively:
\begin{subequations} \label{eq:QOIval}
\begin{align}
\label{eq:idval}
\IDVAL(\bx,n,\mX,\by) &= \SURRERR(n,\mX,\by)(\bx), \\
\label{eq:minval}
\MINVAL(\bx,n,\mX,\by) &= - [\SURR(n,\mX,\by)(\bx) - \SURRERR(n,\mX,\by)(\bx)].
\end{align}
\end{subequations}
The next data site, $\bx_{n+1}$ \emph{maximizes} the acquisition function (see \eqref{eq:nextsample}). For function approximation we sample next where the surrogate model uncertainty is greatest. For optimization we sample next where the output might be smallest.
...
The algorithm that we will develop and analyze is sketched in Algorithm \ref{alg:algframe}.  Some of the ideas in this algorithm have been described earlier in this section.  The more novel ones are described in the next section.

\begin{algorithm}
	\caption{Adaptive Algorithm to Approximate $\QOI(f)$ \label{alg:algframe}}
	\begin{algorithmic}
		\PARAM a candidate set (cone) of input functions, $\cc$; the finite-dimensional space, $\cP$, for MLS approximation of the trend; the MLS weight function; the parameterized reproducing kernel  $K_{\btheta}$; an initial sample size, $n_0$
		...
		\State Compute the variation, $\SVAR(n,\mX,\by)$, as a minimum norm RKHS interpolant with optimal $\btheta$ \\ \qquad \quad  according to Sects.\ \ref{sec:varmodel}, \ref{sec:uncertVar} and \ref{sec:kerinferdata}
		...
		\State Construct error bound $\ERR(n,\mX,\by)$ for approximation $\APP(n,\mX,\by)$ via \eqref{eq:QOIerr} using the \\
		\qquad \quad surrogate model, $\SURR(n,\mX,\by) = \STREND(n,\mX,\by) + \SVAR(n,\mX,\by)$, and its uncertainty, $\SURRERR(n,\mX,\by)$
		...
		\State Choose the next data site $\bx_{n+1}$ according to \eqref{eq:nextsample}, where the acquisition function, $\VAL(\mX,\by)$, \\ \qquad \quad is defined in \eqref{eq:QOIval}
...
Before fleshing out the Algorithm \ref{alg:algframe}, we review some alternative  approaches.
...
\begin{description}
	\item[Discrepancy] This measures the difference between the empirical distribution of the design and the target (uniform, Chebyshev, etc.) distribution \cite{FangEtal19a}. Examples of low discrepancy designs are given in Fig.\ \ref{PtsFig}. 
...
Although uniform grids fill space, they suffer from the curse of dimensionality to do so. 
Sparse grids \cite{BunGrie04a} are an improvement, but the number of points required can still be impractical in high dimensions. 
Orthogonal arrays and Latin hypercube sampling are other examples of space-filling designs, although they are not usually constructed sequentially. 

A major disadvantage of space-filling sampling is that it does  not take advantage of the available function data.  This is appropriate in the exploration stage when we have little, or no, information of $f$, but not in the exploitation stage.
...
\begin{description}
	...
	\item[Smolyak Algorithms \cite{BunGrie04a}] They are based on sparse grids and have the associated weaknesses mentioned above.  Moreover, any algorithm based on a grid will be challenging if we want to rotate the coordinate system to identify a low dimensional structure.  This is why we use meshless methods.
...
The MLS quasi-interpolants are an effective alternative to piecewise polynomials in high dimensions. MLS uses local low-degree polynomials to capture global nonpolynomial behavior. The additional computational cost to solve for the local MLS coefficients is small compared to the cost of generating new function data. 

One of the challenges in MLS is to identify a robust methodology for picking the appropriate length scale, $\sigma(\bx)$, for the MLS kernel \cite{salehi2013generalized,mederos2003moving,cheng2008survey}. Our current approach is to define this length scale as the mean distance to the nearest $\kappa_n$ neighbors of $\bx$, where $\kappa_n$ is greater than $\dim(\calp)$, the dimension of the space of local approximants.
...
\begin{subequations} \label{eq:DataErrBd}
	...
	\end{gather}
\end{subequations}
This idea of data-driven error bounds for numerical problems has been proposed and studied by \FH, \YD, and their collaborators \cite{HicEtal14a, HicEtal14b, HicJim16a, JimHic16a, HicEtal17a, ChoEtal17a}.
...
\end{equation}
where $\pi_d$ is a permutation of the integers $1, \ldots, d$.  The reason that we choose some coordinates to be less important than others is to ensure a low essential dimension.  The reason for the permutation of coordinate indices is to illustrate how our process does not require the relative importance of the coordinates to be known in advance.
...
The exploratory design is $n_0 = 16$ uniform lattice points.  For performing the sup-norm calculations, we use $\ct$ consisting of $2^{16}$ uniform lattice points.

The adaptive algorithm successfully computes the approximation, $\APPID(n,\mX,\by_\VAR)$, which satisfies the sequence of error tolerances via adaptive sampling and an data-driven error bound, $\ERR(n,\mX,\by_\VAR) = \norm[\infty]{\SVARERR(n,\mX,\by_{\VAR})}$.  We observed that the pointwise data-driven error bound in  \eqref{eq:DataErrBd} was successful for $100\%$ of the points in $\ct$. Moreover, the inferred $\theta_\ell$ generally decreased as the $\alpha_\ell$ decreased.
...
The weighted Bayesian bootstrap algorithm will start with an ensemble of $B$ trend models by extending \eqref{eq:MLS}:
\begin{equation}
	\STREND^b(n,\mX,\by)(\bx) = f^b_{\LS,\bx}(\bx), \qquad \text{where } f^b_{\LS,\bx} = \argmin_{g \in \calp} \sum_{i=1}^n v_i^b w_i(\bx)(y_i - g(\bx_i))^2~~.
\end{equation}
Here as in Sect.\ \ref{sec:trend}, the $w_i$ are the MLS weight functions, and now $\bv^b$ is an $n$-vector uniformly distributed over the unit simplex, $\cals^n : = \{\bv \in [0,1]^n : v_1 + \cdots + v_n = 1\}$. We then define the bootstrap trend data as $\by^b_{\TREND} = \bigl(\STREND^b(n,\mX,\by)(\bx_i) \bigr)_{i=1}^n$. 
...
\end{equation}
We are not certain if this simple estimate is the best measure of uncertainty. We will explore alternatives to identify possible better approaches to \emph{combine the ensemble of uncertainty estimates}.
...
\item[Optimization] We will use  surrogate models to identify multiple local optima of the response function. The local minima can then be used to identify as starting locations for gradient descent algorithms.
\end{description}
\fi
...
Hunke first conducted 150 CICE model simulations, selected to space-filling in the 39-dimensional space of feasible parameters. 
...
We will collaborate with Kostas Tsigaridis and Allegra LeGrande to help guide large-scale simulations using the Goddard Institute for Space Studies (GISS) large-scale climate model E2.1-R. \emph{Our goal will be to identify which new simulations will provide the most useful information to predict the impact of volcanic eruptions on the climate.} Last year, \MH mentored Helen Weierbach's undergraduate honor's thesis at Tulane using our surrogate model for some preliminary studies based on Weierbach's was undergraduate research internship at GISS with Tsigaridis and LeGrande. We will continue this research to quantify the sensitivity of GISS model predictions for climate change to the North Atlantic Oscillation. 
...
For the past seven years, \MH has been collaborating with the Tulane School of 
...
\emph{We will use adaptive sampling to optimize the mitigation strategies for slowing the spread of STIs.} 
...
\Upara{Mentoring}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\FH and \MH lead regular research group meetings at their respective institutions comprised of long-term and short-term student collaborators, visitors, the curious, and special guests. Ongoing work in early or polished stages is shared. Papers written by others are presented. Mentoring takes place during these meetings, as well as individually.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{sloppypar}\Upara{Providing Research Experiences for Undergraduate and High School Students} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Our project will support two summer undergraduate students per year at Illinois Tech, or at Los Alamos National Laboratory on the sea ice project. As in the past, the NSF funds will be a catalyst for seeking additional funds to support summer students. In choosing summer students, we will make a deliberate effort to diverse research environment by targeting female and underrepresented minority students as well as students from less research-focused institutions (see Sect.~\ref{prevBIsect}). We will also welcome well-prepared high school students to join our research group.\end{sloppypar}
...
We maintain contact with many of our former students, in particular Yiou Li (female, DePaul U assistant professor). We will continue to help our students prepare for academic careers and continue mentoring them after they leave Illinois Tech.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
We will continue providing lectures to students at various stages in their careers, ranging from high school to graduate school. These encourage students to enter STEM and encourage STEM students to engage in research.
...
\subsection{Hickernell: NSF-DMS-1522687\except{toc}{, \emph{Stable, Efficient, Adaptive Algorithms for
...
\Upara{Publications, Conference Participation, Conference Organization, and Leadership} Publications by \GEF, \FH,  \SCTC, students, and collaborators are listed above.  
The project personnel spoke at many academic conferences and gave colloquium/seminar talks to mathematics and statistics departments.  \FH co-organized the 2016 Spring Research Conference.   \FH gave an invited tutorial
at MCQMC 2016
\cite{Hic17a}, a biennial conference for which he serves on the steering committee.  \FH
was a program leader for the SAMSI 2017--18 Quasi-Monte Carlo (QMC) Program.   \FH received the 2016 Joseph F.\ Traub Prize for Achievement in Information-Based Complexity.  \FH was appointed the director of Illinois Tech's new Center for Interdisciplinary Scientific Computation in 2017.  In 2018, \FH was appointed Vice Provost for Research.
...
\Upara{Boosting the STEM Workforce} \GEF, \FH, and \SCTC mentored research students associated with 
this project.  Female students include \YD, \LJ, \JL, \XT, and Xiaoyang Zhao (MS 2017).   Mentees include more than a dozen
...
\subsection{Hyman: NSF-DMS-1563531\except{toc}{, \emph{Multiscale Models for Predicting the Effectiveness of Mitigation Efforts in Controlling Vector-Borne Epidemics},
	\$434,289, May 2016 -- April 2019}
} \label{sec:PriorMac}
\MH was PI on this award. \textit{ Publications:} \cite{xue2016two,xue2018comparing,qu2018modeling,moran2016epidemic,chowell2016mathematical,margevicius2016biosurveillance}

\Upara{Intellectual Merit} We analyzed a new class of mathematical models to forecast the spread of emerging mosquito-borne diseases.  The model included behavior change in the human population and the impact of infecting wild mosquitoes with Wolbachia in mitigating these diseases.

\Upara{Broader Impact}   We mentored several undergraduate capstone projects, honor's thesis and graduate Master's dissertations and two PhD dissertations in mathematics and at the Tulane School for Public Health and Tropical Medicine on the modeling of mosquito-borne diseases. 
