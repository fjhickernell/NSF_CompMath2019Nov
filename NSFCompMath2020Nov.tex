...
\begin{equation} \label{eq:cos_sum}
	f_V(\bx) =  \cos( \alpha_1 x_1 + \cdots + \alpha_d x_d), \qquad \alpha_j = 2^{-\pi_d(\ell)}, \ell = 1, \ldots, d,
\end{equation}
...
Table  \ref{tab:cos_sum} is the output of a stripped-down version of our algorithm in Sect.\ \ref{sec:algo} without the trend. We take $\SVARERR(n,\mX,\by_{\VAR})$ to be the uncertainty in the surrogate model.  For $A(\mX)$ we use
...
The exploratory design is $n_0 = 10$ uniform lattice points.    
