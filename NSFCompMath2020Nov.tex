...
\begin{description}
\item[Multivariate polynomials] are too inflexible because they limit the approximation to a finite dimensional space, and they must be of low degree in high dimensions to avoid the curse of dimensionality.
\item[Smolyak Algorithms ] based on sparse grids have the weaknesses mentioned above for sparse grids.  Moreover, any algorithm based on a grid will have challenges if we want to rotate the coordinate system to identify a low dimensional structure.
\item[Tensor Trains] Maximin designs maximize this criterion \cite{jin2002sequential}.
\item[Entropy] Designs maximizing entropy seek to maximize the information when choosing the next data site \cite{jin2002sequential}. They also typically assume a known Gaussian process model.
\end{description}
Multivariate polynomials are too inflexible because they limit the approximation to a finite dimensional space, and they must be of low degree in high dimensions to avoid the curese of dimensionality.
