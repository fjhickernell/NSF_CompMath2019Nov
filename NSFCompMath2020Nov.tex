...
\voffset 0.25in  %Made to satisfy Research.gov compliance
\hoffset 0.01in
\textheight 8.91in
\textwidth 6.46in
...
The trend model can be based on a simple approximation for the QOIs, such as a reduced model or a global multivariate polynomial.
...
To mitigate this curse we choose our candidate set $\calc$ to only include functions with \emph{low effective dimension}, $d_\eff < d$, which our algorithm can identify and where $\ERR(n,\mX,\by) = \Order(n^{-r/d_\eff})$.  This can be done via coordinate weights or active subspaces (Sect.\ \ref{sec:kerinferdata}). 
...
We will compare the effectiveness of low discrepancy sampling for uniform and Chebyshev (arcsine) distributions.
The Chebyshev distribution pushes points towards the boundary of $\Omega$ so that there is sufficient data to interpolate $f$ there.  The advantage of doing so may depend on the smoothness of $f$ \cite{HicLi12a}.
...
\Upara{Intellectual Merit} We created and analyzed a new class of mathematical models to forecast the spread of emerging mosquito-borne diseases.  The new model included behavior change in the human population and the impact of infecting wild mosquitoes with Wolbachia in mitigating these diseases.
