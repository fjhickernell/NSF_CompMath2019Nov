...
While  \eqref{eq:DataErrBd} may provide a data-driven error bound for the surrogate variation, there still remains the question of which kernel to choose.  For example, the squared exponential kernel in \eqref{eq:GaussKer} has the parameter $\btheta$ that must be specified.

We propose to experiment with the empirical Bayes perspective, which leads to the following choice \cite{Hic17a}: 
\begin{equation} \label{eq:thetEB}
	\theta_{\textup{EB}} = \argmin_\btheta \left[\frac 1n \log \bigl( \det(\mK_\btheta) \bigr) + \log \bigl ( \by^T \mK_\btheta^{-1} \by \bigr)\right].
\end{equation}
...
\begin{equation} \label{eq:cos_sum}
	f_{\VAR}(\bx) =  \cos( \alpha_1 x_1 + \cdots + \alpha_d x_d), \qquad \alpha_j = 2^{-\pi_d(j)}, j = 1, \ldots, d,
\end{equation}
where $\pi_d$ is a permutation of the integers $1, \ldots, d$.  The reason that we choose some coordinates as less important than others is to illustrate a low essential dimension.  The reason for the permutation is to illustrate how our process does not require a priori knowledge of the relative importance of the coordinates.

Table  \ref{tab:cos_sum} is the output of applying a stripped-down version of our adaptive algorithm in Sect.\ \ref{sec:algo} without the trend. We take $\SVARERR(n,\mX,\by_{\VAR})$ to For $A(\mX)$ we use
...
The exploratory design is $n_0 = 10$ lattice points.    

The adaptive algorithm successfully computes the approximation, $\APPID(n,\mX,\by)$, which satisfies the sequence of error tolerances via adaptive sampling and an data-driven error bound, $\ERR(n,\mX,\by) = \SVARERR(n,\mX,\by_{\VAR})$ 
...
\begin{array}{rccccccc} 
	\text{Coordinate, } j& 9 & 3 & 2 & 8 & 11 & 12 \\
	\alpha_j & 5.0\text{E}{-}1 & 2.5\text{E}{-}1 & 1.3\text{E}{-}1  & 6.3\text{E}{-}2 &3.1\text{E}{-}2& 1.6\text{E}{-}2 \\
	\theta_j & 3.9\text{E}{-}1 & 1.7\text{E}{-}1 & 1.1\text{E}{-}1  & 6.5\text{E}{-}2 &3.8\text{E}{-}2&  2.1\text{E}{-}2 \\
	\hline
	\text{Coordinate, } j & 4 & 10 & 7 & 1 & 6 & 5 \\
	\alpha_j & 7.8\text{E}{-}3 & 3.9\text{E}{-}3 & 2.0\text{E}{-}3 & 9.8\text{E}{-}4 & 4.9\text{E}{-}4 & 2.4\text{E}{-}4\\
	\theta_j & 1.2\text{E}{-}2 & 8.3\text{E}{-}3 &  5.7\text{E}{-}3 &  2.9\text{E}{-}3 & 2.0\text{E}{-}3&  1.7\text{E}{-}3\\
	\hline
	...
	\SVARERR(n,\mX,\by_{\VAR})  & 9.5\text{E}{-}2 & 4.8\text{E}{-}2 & 1.9\text{E}{-}2 & 9.5\text{E}{-}3 & 4.8\text{E}{-}3 & 1.8\text{E}{-}3 & 9.6\text{E}{-}4 \\ \hline 
	\rule[3ex]{0pt}{0pt} \bignorm[\infty]{f_{\VAR} - \APPID(n,\mX,\by_{\VAR})} & 2.1\text{E}{-}2 & 9.6\text{E}{-}3 & 3.7\text{E}{-}3 & 1.9\text{E}{-}3 & 8.0\text{E}{-}4 & 3.7\text{E}{-}4 & 1.8\text{E}{-}4 \\ \hline 
