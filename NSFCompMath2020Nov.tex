...
For an error bound of the form \eqref{eq:errbd}  to even be possible, the $\QOI$ of any two functions in $\calc$ sharing the same data must not differ by much, specifically, 
...
We assume that $\calc$ is a subset of $\calf$ an infinite dimensional vector space of $d$-variate functions for which our approximation,  $\APP(n,\mX,\by)$, comes with an error bound $\ERR(n,\mX,\by)$. We cannot choose $\calc = \calf$ because then there would surely be some $g \in \calc$ with $\QOI(g)$ arbitrarily far away from $\QOI(f)$.  E.g., $g$ could have all the smoothness desired but be quite peaky.

The candidate set must only include functions whose norms can be well approximated from the data, $(\mX, \by)$.  Otherwise, again there would be a peaky $g \in \calc$ with large $\norm[\calf]{g}$ and $\QOI(g)$ arbitrarily far away from $\QOI(f)$.  While some might choose $\calc$ to include only functions with $\calf$-norm no greater than $R$, we find this artificial, and will choose $\calc$ to include functions with arbitrarily large $\calf$-norm.

For our problems of interest there is a possible curse of dimensionality, meaning that the convergence rate of $\ERR(n,\mX,\by)$ is $\Order(n^{-r/d})$ for some fixed $r$ depending on the smoothness of $f$.  This  is disastrous for large $d$.  The only way to avoid this curse is for our algorithm to identify a subspace of $\calf$ with low effective dimension, $d'$, where $\ERR(n,\mX,\by) = \Order(n^{-r/d'})$.  This can be done via coordinate weights or active subspaces (see Set.\ \ref{sec:selectCoord}).  Our $\calc$ will include only functions with low effective dimension.

Although the candidate set, $\calc$, must be small enough, we also want it be as large as possible to give our algorithm wider applicability.  To do this, we will allow $\calf$ to be the union of build into our algorithm the flexibility to choose among different possible low dimensional structures in different possible vectors spaces, $\calf_\btheta$.
