...
\item $f$ is function, such as a large-scale computer simulation, that generates the information required for the \QOIs. 
\item $\by := \bigl(f(\bx_1), \ldots, f(\bx_n) \bigr)^T \in \reals^n$ is a vector of (noiseless\footnote{Noisy data is an important case for future study.}) \emph{output function data}.
...
This function, $\VAL$, relies on a pointwise error bound for the surrogate model and depends on both the surrogate and  uncertainty models.  For example, when minimizing a function, $\VAL$ corresponds to the expected improvement in the approximation to the minimum. The precise definitions are given in Sect.\ \ref{sec:acquire}.
...
\item Better adaptive sampling acquisition functions, $\VAL$, whic depend on both the past sample locations, $\mX$, and the output data, $\by$, (Sect.\ \ref{sec:acquire}),
...
Interpolating between sample locations is more reliable than extrapolating outside their convex hull.  The Chebyshev (arcsine) distribution pushes points towards the boundary of $\Omega$, and the convex hull of the resulting distribution of samples includes more of $\Omega$ than uniform sampling. We will compare the effectiveness of low discrepancy sampling for Chebyshev and uniform distributions to see which approach is the most effective in defining the surrogate model. 
...
\end{equation}
and $\calp$ is a finite dimensional vector space, e.g., low degree polynomials. The weights $w_i(\bx)$ decrease with distance $\norm[2]{\bx - \bx_i}$. We will initially use a Gaussian kernel weight, $w_i(\bx) = \me^{-\norm[2]{\bx - \bx_i}^2/\sigma(\bx)^2}$, where $\sigma(\bx)$ is the mean distance from the evaluation point, $\bx$, to a set of $K_n$ nearest neighbors \cite{cheng2008survey}. The number of nearest neighbors will be greater than $\dim(\calp)$.
...
A major disadvantage of space-filling sampling is that they do  not take advantage of the available function data.  This is appropriate in the exploration stage when we have little, or no, information of $f$.

%Grids do not fill space, are not sequential, and are not efficient in high dimensions.  
...
One of the challenges in MLS is to identify a robust methodology for picking the appropriate length scale, $\sigma(\bx)$, for the MLS kernel \cite{salehi2013generalized,mederos2003moving,cheng2008survey}. Our current approach is to define this length scale as the mean distance to the nearest $K_n$ neighbors of $\bx$, where $K_n$ is greater than $\dim(\calp)$, the dimension of the space of local approximants.
...
\begin{subequations} \label{eq:DataErrBd}
	...
	\end{gather}
\end{subequations}
This idea of data-driven error bounds for numerical problems has been studied by \FH, \YD, and their collaborators \cite{HicEtal14a, HicEtal14b, HicJim16a, JimHic16a, HicEtal17a, ChoEtal17a}.
...
We maintain contact with many of our former students, in particular Yiou Li (female, DePaul assistant professor). We will continue to help our students prepare for academic careers and continue mentoring them after they leave Illinois Tech.
